# https://github.com/harubaru/waifu-diffusion/blob/main/train.sh
import uuid
...
class DataModuleFromConfig(pl.LightningDataModule):
    def __init__(self, batch_size, train=None, husky=None, validation=None, test=None, predict=None,
        ...
        #     )
        self.__train_dataloader = self._husky_dataloader(1000)
        self.__val_dataloader = self._husky_dataloader(10)
        self.datasets = {
            'train': self.__train_dataloader,
            'validation': self.__val_dataloader
        }
    ...
    def _husky_dataloader(self, max_entries):
        data_path = abspath(os.path.join(os.path.dirname(__file__), './data'))

        husky_dataset = HuskyDataset(data_path)

        bucket = HuskyBucket(data_path, num_buckets=101, batch_size=self.batch_size, max_ratio=2, max_entries=max_entries, max_image_area=512 * 768)

        custom_sampler = HuskyBatchSampler(bucket)

        return DataLoader(husky_dataset, batch_sampler=custom_sampler, num_workers=self.num_workers, shuffle=False)
...
if __name__ == "__main__":
    ...
    try:
        ...
        signal.signal(signal.SIGUSR1, divein)
